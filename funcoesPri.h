...
void configuraEmail(char *nome_arquivo){
    char email[300], emailConfer[300], opcao; 
    int emailOk = 0;

    limpaTela();
    printf("CONFIGURAR EMAIL\n");
    printf("----------------\n\n");

    FILE *f = fopen(nome_arquivo, "r+t");
    if (f==NULL){
        ...
        while (!emailOk){
            ...
            scanf(" %[^\n]", emailConfer);
            if (strcmp(email, emailConfer) == 0){
                emailOk = 1;
            }else printf("Email nao confere. Digite novamente.\n");
        }
        ...
        do {
            scanf(" %c", &opcao);
            if (opcao != 's' && opcao != 'n') {
                printf("Opcao inválida, digite novamente [s/n]: ");
            }
